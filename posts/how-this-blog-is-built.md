...
When I came to build another blog, static site generators were becoming popular. This idea seemed much more suited to my use case. With a static site generator blog posts could simply be written in markdown, converted to static HTML, CSS and JavaScript and hosted cheaply without the need for a backend or a database. Aside from the cost saving, having no backend means there's nothing for hackers to target too! I built a blog using a popular Ruby-based generator called <a href="https://jekyllrb.com/" class="link">Jekyll</a>, which made getting things up and running really simple. However, over time, even Jekyll seemed like more than I really needed for my basic blogging requirements.
